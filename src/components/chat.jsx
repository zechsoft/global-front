...
import {
  Send, Paperclip, Smile, Phone, Video, MoreVertical, Search, Plus,
...
export default function ChatPage() {
  ...
  const loadConversations = useCallback(async () => {
    ...
    try {
      ...
      const data = await authenticatedApiCall(`${API_BASE}/chat/conversations`);
  ...
  const loadChatRooms = useCallback(async () => {
    ...
    try {
      ...
      const data = await authenticatedApiCall(`${API_BASE}/chat/rooms`);
      console.log('Chat rooms loaded:', data.chatRooms?.length || 0);
      setChatRooms(data.chatRooms || data.rooms || []);
    } catch (error) {
  ...
  const loadClients = useCallback(async () => {
    ...
    try {
      ...
      const response = await authenticatedApiCall(`${API_BASE}/get-clients`);
      console.log('Clients loaded:', response.users?.length || 0);
  ...
  const loadAdmins = useCallback(async () => {
    ...
    try {
      ...
      const response = await authenticatedApiCall(`${API_BASE}/get-admin`);
      console.log('Admins loaded:', response.users?.length || 0);
  ...
  const loadAllUsers = useCallback(async () => {
    ...
    try {
      ...
      const response = await authenticatedApiCall(`${API_BASE}/get-all-users`);
